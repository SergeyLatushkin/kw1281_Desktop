...
namespace BitFab.KW1281Test.Cluster
{
    class MarelliCluster : ICluster
    {
        ...
        private ushort GetDefaultAddress()
        {
            ...
            {
                Mc.AddLine();
                Mc.AddLine("Unsupported Marelli cluster version.");
                Mc.AddLine("You can try the following commands to see if either produces a dump file.");
                Mc.AddLine("Then please contact the program author with the results.");
                Mc.AddLine();

                var prefix = string.Join(' ', Diagnostic.CommandAndArgs.Take(4));
                Mc.AddLine($"{prefix} DumpMarelliMem 3072 1024");
                Mc.AddLine($"{prefix} DumpMarelliMem 14336 2048");
            ...
            ushort? count = null)
        {
            ...
            {
                Mc.AddLine("Untested cluster version! You may need to disconnect your battery if this fails.");
            ...
            {
                Mc.AddLine("Unsupported cluster software version");
                return [];
            }

            Mc.AddLine($"entryH: 0x{entryH:X2}, regBlockH: 0x{regBlockH:X2}, count: 0x{count:X4}");

            Mc.AddLine("Sending block 0x6C");
            _kwp1281.SendBlock([0x6C]);

            Thread.Sleep(250);

            Mc.AddLine("Writing data to cluster microcontroller");
            var data = new byte[]
            {
            ...
            Mc.AddLine("Writing memory dump program to cluster microcontroller");
            Mc.AddLine($"(Entry: ${entryH:X2}00, RegBlock: ${regBlockH:X2}00, Start: ${address:X4}, Count: ${count:X4})");
            ...
            Mc.AddLine("Receiving memory dump");
            ...
            Mc.AddLine($"Saved memory dump to {filename}");
        ...
        private bool WriteMarelliBlockAndReadAck(byte[] data)
        {
            ...
            {
                Mc.AddLine($"Expected ACK but received {Utils.Dump(ack)}");
                return false;
            }
