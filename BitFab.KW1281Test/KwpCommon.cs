...
namespace BitFab.KW1281Test
{
    ...
    internal class KwpCommon : IKwpCommon
    {
        ...
        public int WakeUp(byte controllerAddress, bool evenParity, bool failQuietly)
        {
            ...
            if (!noGC)
            {
                Mc.AddLine("Warning: Unable to disable GC so timing may be compromised.");
            }
            ...
            try
            {
                ...
                for (var i = 1; i <= maxTries; i++)
                {
                    ...
                    {
                        Mc.AddLine(ex.Message);

                        if (i < maxTries)
                        {
                            Mc.AddLine("Retrying wakeup message...");
                        ...
                        else
                        {
                            if (!failQuietly)
                            {
                                Mc.Add("Controller did not wake up.");
                                Mc.Add("    - Are you using a supported cable?");
                                Mc.Add("    - Is the cable plugged in and any necessary drivers installed?");
                                Mc.Add("    - Is the ignition on?");
                                Mc.Add("    - Is the controller address correct?");
                                Mc.Add("    - Is the baud rate correct (unexpected sync byte errors)? Try 10400, 9600, 4800.");
                                Mc.Add("You can try other software (e.g. VCDS-Lite) to verify that the cable/drivers/address are ok.");
                            }
        ...
        private int WakeUpNoRetry(byte controllerAddress, bool evenParity)
        {
            ...
            if (syncByte != 0x55)
            {
                throw new InvalidOperationException($"Unexpected sync byte: Expected $55, Actual ${syncByte:X2}");
            }
            ...
            {
                ReadComplement(Utils.AdjustParity(controllerAddress, evenParity));
            }
