...
namespace BitFab.KW1281Test.Cluster
{
    internal class MotometerBOOCluster : ICluster
    {
        ...
        public void UnlockForEepromReadWrite()
        {
            ...
            {
                Mc.AddLine("Warning: Unknown software version. Login may fail.");
                login = 11899;
            }

            _kwp1281!.Login(login, workshopCode: 0);

            Mc.AddLine($"Sending Custom $08 $15 block");
            if (SendCustom(0x08, 0x15))
            {
                return;
            }

            Mc.AddLine("$08 $15 failed. Trying all combinations (this may take a while)...");

            for (int first = 0; first < 0x100; first++)
            {
                Mc.AddLine($"Trying ${first:X2} $00-$FF");

                for (int second = 0; second < 0x100; second++)
                {
                    if (SendCustom(first, second))
                    {
                        Mc.AddLine($"Combination ${first:X2} ${second:X2} Succeeded.");
            ...
            Mc.AddLine("All combinations failed. EEPROM access will likely fail.");
        }
