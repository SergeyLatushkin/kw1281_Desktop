...
namespace BitFab.KW1281Test.Interface.EDC15
{
    public class Edc15VM
    {
        ...
        public byte[] ReadWriteEeprom(string filename, List<KeyValuePair<ushort, byte>>? addressValuePairs = null)
        {
            addressValuePairs ??= [];

            var kwp2000 = new KW2000Dialog(_kwpCommon, (byte)_controllerAddress);

            kwp2000.SendReceive(DiagnosticService.startDiagnosticSession, [0x89]);
            kwp2000.SendReceive(DiagnosticService.startDiagnosticSession, [0x85]);
            ...
            kwp2000.SendReceive(DiagnosticService.securityAccess, buf.ToArray());

            var loader = GetLoader();
            var len = loader.Length;

            // Ask the ECU to accept our loader and store it in RAM
            kwp2000.SendReceive(DiagnosticService.requestDownload, [
            ...
            while (true)
            {
                ...
                kwp2000.SendReceive(
                    ...
                    blockBytes.Take(readCount).ToArray(),
                    excludeAddresses: true);
            }
            ...
            Mc.Add($"Saved EEPROM to {filename}");
            ...
            foreach (var addressValuePair in addressValuePairs)
            {
                ...
                Mc.Add($"Sent: {address:X2} {value:X2}");
            ...
            kwp2000.SendMessage((DiagnosticService)0xA2, [], excludeAddresses: true);
            kwp2000.ReceiveMessage();

            var b = _kwpCommon.Interface.ReadByte();
            if (b == 0x55)
            {
                Mc.AddLine($"Reboot successful!");
            }
        ...
        public static void DisplayEepromInfo(ReadOnlySpan<byte> eeprom)
        {
            ...
            mc.AddLine($"Immo is {immoStatus} (${immo1Addr:X3}=${immo1:X2}, ${immo2Addr:X3}=${immo2:X2})");
        }
